...
namespace Assets.TeamProjects.GamePrimal.SeparateComponents.MiscClasses
{
    public class CursorChanger : IUserAwake
    {
        ...
        public void SetSmartCursor(Transform softFocus, Transform hardFocus, StatesList states, Vector3 touchPoint)
        {
            MonoAmplifierRpg monoRpg = hardFocus ? hardFocus.GetComponent<MonoAmplifierRpg>() : null;

            if (monoRpg != null && softFocus)
            {
                WeaponMathAbs wma = monoRpg.AcquireWMath().SetTouchPoint(touchPoint);

                if (monoRpg.WieldingWeapon.isRanged && wma.InRange())
                    SetShootCursor();
                else if (!monoRpg.WieldingWeapon.isRanged && wma.InRange())
                    SetAggressiveCursor();
                else
                    SetCursorIfOnUi(softFocus, hardFocus, states);
            }
            else
            {
                SetDefaultCursor();
